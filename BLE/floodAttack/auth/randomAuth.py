...
async def run_test():
    ...
    with contextlib.redirect_stdout(session_output):
        ...
        try:
            ...
            iterations = 1000  # Set how many random test cases you want

            for i in range(1, iterations + 1):
                # Randomize full 7-byte AUTH command
                command = [random.randint(0x00, 0xFF) for _ in range(7)]
                ...
                try:
                    ...
                    print(f"[{now}] [TEST {i:03}] AUTH={command[0]:#04x} + {command[1:]} → Response: {res}")
                    if res and res[0] == 0:
                        success += 1
                    else:
                        failures += 1
                except Exception as e:
                    print(f"[{now}] [TEST {i:03}] AUTH={command[0]:#04x} + {command[1:]} → Exception: {e}")
                    errors += 1
